...
def generate_employee(id, role):
    tmp = db_model.Employee(id,  fk.first_name(), fk.last_name(), fk.date_this_decade() ,fk.date_of_birth(minimum_age=18, maximum_age=60) ,random.randint(100000000, 999999999), fk.email(), role ,random.randint(0, CITIES_AMOUNT), random.randint(0, COUNTRIES_AMOUNT))
...
def generate_employees_table():
    ...
    for translator in translators_table:
        ...
        for i in range(lang_count):
            tmp_lang_id = random.randint(0,LANGUAGES_AMOUNt)
